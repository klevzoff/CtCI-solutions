 ...
 */
bool test(BinaryTree<int> const & tree, int a, int b, int c)
{
  BinaryTree<int>::Node const * na = tree.find(a);
  BinaryTree<int>::Node const * nb = tree.find(b);
  BinaryTree<int>::Node const * nc = tree.find(c);
  return nc == common_ancestor<int>(tree, na, nb);
}

int main()
{
  assert(test({{0,-1,-1,0}}, 0, 0, 0));
  assert(test({{0,1,-1,0},{1,-1,-1,1}}, 0, 1, 0));
  assert(test({{0,1,-1,0},{1,-1,-1,1}}, 1, 0, 0));
  assert(test({{0,-1,1,0},{1,-1,-1,1}}, 0, 1, 0));
  assert(test({{0,-1,1,0},{1,-1,-1,1}}, 1, 0, 0));
  assert(test({{0,1,2,0},{1,-1,-1,1},{2,-1,-1,2}}, 1, 2, 0));
  assert(test({{0,1,2,0},{1,-1,-1,1},{2,-1,-1,2}}, 2, 1, 0));
  assert(test({{0,1,2,0},{1,-1,-1,1},{2,3,4,2},{3,-1,-1,3},{4,5,-1,4},{5,-1,-1,5}}, 1, 2, 0));
  assert(test({{0,1,2,0},{1,-1,-1,1},{2,3,4,2},{3,-1,-1,3},{4,5,-1,4},{5,-1,-1,5}}, 1, 3, 0));
  assert(test({{0,1,2,0},{1,-1,-1,1},{2,3,4,2},{3,-1,-1,3},{4,5,-1,4},{5,-1,-1,5}}, 1, 4, 0));
  assert(test({{0,1,2,0},{1,-1,-1,1},{2,3,4,2},{3,-1,-1,3},{4,5,-1,4},{5,-1,-1,5}}, 1, 5, 0));
  assert(test({{0,1,2,0},{1,-1,-1,1},{2,3,4,2},{3,-1,-1,3},{4,5,-1,4},{5,-1,-1,5}}, 2, 4, 2));
  assert(test({{0,1,2,0},{1,-1,-1,1},{2,3,4,2},{3,-1,-1,3},{4,5,-1,4},{5,-1,-1,5}}, 2, 5, 2));
  assert(test({{0,1,2,0},{1,-1,-1,1},{2,3,4,2},{3,-1,-1,3},{4,5,-1,4},{5,-1,-1,5}}, 3, 5, 2));
